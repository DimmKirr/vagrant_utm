    ...
    module Action
      ...
      class PromptVirtioFS
        ...
        def apple_vm?(machine)
          # Check if this is an Apple Virtualization VM by reading backend
          return false unless machine.id

          begin
            script_path = machine.provider.driver.instance_variable_get(:@script_path)
            script = script_path.join("read_shared_folders.js")
            cmd = ["osascript", "-l", "JavaScript", script.to_s, machine.id]
